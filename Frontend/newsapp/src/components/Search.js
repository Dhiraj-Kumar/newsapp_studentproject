...
function Dashboard() {
 ...
 const [searching,setSearching]=useState([]);
  ...
  const DataSearching =async()=>{
    let { data } = await axios.get(`${config.newsapi}/search/${params.value}`);
    setSearching(data);
  ...
  return (
    <div>
      ...
      <div className="container">
        <h2 className="head-text text-center     shadow bg-dark text-light">All news related to {params.value}</h2>
